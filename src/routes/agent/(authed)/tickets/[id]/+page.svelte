...
<div class="space-y-6">
	<div class="flex items-center gap-3">
		<a href="/agent/tickets" class="text-sm text-gray-500 hover:text-gray-700">&larr; Back</a>
	...
	<div class="rounded-lg border border-gray-200 bg-white p-6">
		<div class="flex flex-wrap gap-4">
			...
			<div>
				<span class="text-xs uppercase text-gray-500">SLA Status</span>
				<p class="text-sm text-gray-700">{data.ticket.sla_status ?? '-'}</p>
			</div>
			<div>
	...
	<div class="rounded-lg border border-gray-200 bg-white">
		...
		<div class="divide-y divide-gray-100 p-6">
			{#each data.messages as message}
				<div
					class="py-4 first:pt-0 last:pb-0 {message.is_internal
						? 'rounded-md border border-yellow-200 bg-yellow-50 px-3'
						: ''}"
				>
					<div class="flex items-center gap-2">
						...
						{#if message.is_internal}
							<span class="rounded-full bg-yellow-200 px-1.5 py-0.5 text-xs text-yellow-800">
								internal
							</span>
						{/if}
		...
		<div class="border-t border-gray-200 p-4">
			...
			>
				...
				<label class="flex items-center gap-2">
					...
					/>
					<span class="text-xs text-gray-500">Internal note (not visible to client)</span>
				</label>
			</form>
		</div>
	</div>
